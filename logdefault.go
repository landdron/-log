...
func (this *fileBeanFactory) add(dir, filename string, _suffix int, maxsize int64, maxfileCount int32) {
	//timefilename := filename + strconv.Itoa(int(time.Now().Month())) + "-"+strconv.Itoa(time.Now()) + "-"+strconv.Itoa(time.Now().Hour)
	this.mu.Lock()
...
func newFileBean(fileDir, fileName string, _suffix int, maxSize int64, maxfileCount int32) (fb *fileBean) {
	fb = &fileBean{dir: fileDir, filename: fileName, hour: time.Now().Hour(), mu: new(sync.RWMutex)}
	nextfilename := "-"+fmt.Sprintf("%1.2d",int(time.Now().Month())) + "-"+fmt.Sprintf("%1.2d",time.Now().Day()) + "-"+fmt.Sprintf("%1.2d",time.Now().Hour()) + ".log"
	fb.logfile, _ = os.OpenFile(fileDir+"/"+fileName+nextfilename, os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	fb.lg = log.New(fb.logfile, "", log.Ltime)
	fb._suffix = _suffix
	fb.maxFileSize = maxSize
	fb.maxFileCount = maxfileCount
	fb.filesize = fileSize(fileDir + "/" + fileName)
	fb.hour = time.Now().Hour()
	return
}
...
func (this *fileBean) rename(rolltype _ROLLTYPE) {
	...
	switch rolltype {
	case _DAILY:
		nextfilename = nextfilename + fmt.Sprintf("%1.2d",int(time.Now().Month())) + "-"+fmt.Sprintf("%1.2d",time.Now().Day()) + "-"+fmt.Sprintf("%1.2d",time.Now().Hour()) + ".log"
	...
	this.hour = time.Now().Hour()
	this.logfile, _ = os.OpenFile(this.dir+"/"+this.filename + nextfilename, os.O_RDWR|os.O_APPEND|os.O_CREATE, 0666)
	this.lg = log.New(this.logfile, "", log.Ltime)
